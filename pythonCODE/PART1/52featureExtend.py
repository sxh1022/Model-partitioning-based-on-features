# # 对特征线进行延伸，封闭特征线，创建新的边

import bpy
import bmesh
import statistics
import mathutils
import json

# 获取活动对象
obj = bpy.context.active_object
# 获取网格数据
mesh = obj.data

# 创建一个BMesh实例
bm = bmesh.new()
bm.from_mesh(mesh)
bm.edges.ensure_lookup_table()
bm.faces.ensure_lookup_table()
bm.verts.ensure_lookup_table()


# ======================================计算每个连通区域到边缘的距离=================================================

# 计算点到最近边界边的距离
def distance_to_boundary(vert, boundary_edges):
    min_distance = float('inf')
    for edge in boundary_edges:
        for v in edge.verts:
            distance = (vert.co - v.co).length
            if distance < min_distance:
                min_distance = distance
    return min_distance


def find_connected_components(bm, edges):
    connected_components = []  # 存储连通区域的列表，每个区域是一组边的集合
    visited_edges = set()  # 存储已访问的边索引

    while edges:
        stack = [edges.pop()]  # 以新的锚边开始

        visited = set()

        while stack:
            edge_idx = stack.pop()
            visited.add(edge_idx)
            visited_edges.add(edge_idx)
            edge = bm.edges[edge_idx]

            # 通过边的共享顶点找到相邻边，但仅添加未访问的边
            adjacent_edges = set()
            for vert in edge.verts:
                adjacent_edges.update(
                    linked_edge.index
                    for linked_edge in vert.link_edges
                    if (
                            linked_edge.index in edges
                            and linked_edge.index not in visited
                            and linked_edge.index not in visited_edges
                    )
                )

            stack.extend(adjacent_edges)
            edges.discard(edge_idx)  # 从边集合中移除已处理的边

        connected_components.append(visited)

    return connected_components


# 获取已选锐边
selected_edges = [edge for edge in bm.edges if edge.select]
selected_edges_index = {edge.index for edge in selected_edges}

selected_vertices = set()
# 收集已选锐边的顶点
for edge in selected_edges:
    selected_vertices.update(edge.verts)

# 收集模型边界的顶点
boundary_edges = [edge for edge in bm.edges if edge.is_boundary]
boundary_vertices = set()
for edge in boundary_edges:
    boundary_vertices.update(edge.verts)

# 获取连通区域
connected_components = find_connected_components(bm, selected_edges_index)

# 创建一个字典来存储连通区域的信息
connected_components_info = []

# 计算每个连通区域到边缘的距离
for index, component in enumerate(connected_components):
    distance = 0.0
    vertex_set = set()
    component_edges = []

    for edge_idx in component:
        component_edges.append(edge_idx)
        for vertex in bm.edges[edge_idx].verts:
            vertex_set.add(vertex)

    for vert in vertex_set:
        distance += distance_to_boundary(vert, boundary_edges)
    distance /= len(component)

    component_info = {
        "index": index + 1,
        "edges": component_edges,
        "distance": distance,
        "weight": 0.0  # 临时设置为0，稍后会更新权重
    }

    connected_components_info.append(component_info)

# 计算权重
max_distance = max(info['distance'] for info in connected_components_info)
min_distance = min(info['distance'] for info in connected_components_info)

# 计算初始权重时，考虑距离和边的数量
for info in connected_components_info:
    if max_distance != min_distance:
        weight = (info['distance'] - min_distance) / (max_distance - min_distance)
    else:
        # 如果最大距离和最小距离相等，默认权重0
        weight = 0.0
    info['weight'] = weight

# 输出每个连通区域的边的数量、距离和权重
for info in connected_components_info:
    print(f"index: {info['index']} len: {len(info['edges'])} distance: {info['distance']} weight: {info['weight']}")


# 定义一个函数来寻找每个连通区域的断点
def find_breakpoints_for_component(bm, component_edges):
    bm.edges.ensure_lookup_table()
    bm.faces.ensure_lookup_table()
    bm.verts.ensure_lookup_table()
    breakpoints = set()
    vert_edge_counts = {}

    for edge_idx in component_edges:
        edge = bm.edges[edge_idx]
        for vert in edge.verts:
            vert_edge_counts[vert] = vert_edge_counts.get(vert, 0) + 1

    for vert, count in vert_edge_counts.items():
        if count == 1:
            breakpoints.add(vert)

    return breakpoints


# 判断两条边是否平行
def are_edges_parallel(edge1, edge2, tolerance=1e-5):
    # 获取边的向量
    vec1 = edge1.verts[1].co - edge1.verts[0].co
    vec2 = edge2.verts[1].co - edge2.verts[0].co

    vec1.normalize()
    vec2.normalize()

    # 计算向量点积
    dot_product = vec1.dot(vec2)

    return abs(dot_product - 1.0) < tolerance


# 判断两个相邻三角面是组成了一个大三角形还是四边形
def is_quadrilateral(face1, face2, shared_edge):
    # 获取两个面的所有边
    edges_face1 = list(face1.edges)
    edges_face2 = list(face2.edges)

    # 检查是否有共享边
    if shared_edge not in edges_face1 or shared_edge not in edges_face2:
        return False

    # 计算各自的非共享边数
    non_shared_edges_face1 = [edge for edge in edges_face1 if edge != shared_edge]
    non_shared_edges_face2 = [edge for edge in edges_face2 if edge != shared_edge]

    # 如果两个面都有一个非共享边，那么它们组成了一个四边形
    return len(non_shared_edges_face1) == 1 and len(non_shared_edges_face2) == 1


# 找到断点延伸的所有可能
def find_all_verts(breakpoint):
    # 存储所有可选择的点
    optional_verts = []

    # 找到当前点的邻接边、邻接点、邻接面
    adjacent_edges = [edge for edge in breakpoint.link_edges]

    adjacent_verts = [e.other_vert(breakpoint) for e in breakpoint.link_edges]
    optional_verts.extend(adjacent_verts)

    # 创建一个集合来存储邻接面
    adjacent_faces = [face for face in breakpoint.link_faces]

    # 获取点的邻接面
    for face in adjacent_faces:
        # 找到邻接面的边（不属于点的邻接边的那条边）
        edges = [e for e in face.edges if e not in adjacent_edges]

        if len(edges) == 1:
            # 找到边的邻接面（不属于点的邻接面的那个面）
            faces = [f for f in edges[0].link_faces if f not in adjacent_faces]

            if len(faces) == 1:
                if not is_quadrilateral(face, faces[0], edges[0]):
                    continue  # 跳过不是四边形的情况
                else:
                    # 找到面的顶点（不属于点的邻接点的那个点）
                    verts = [v for v in faces[0].verts if v not in adjacent_verts]
                    if len(verts) == 1:
                        optional_verts.append(verts[0])

    return optional_verts


# 找到一条边的中点,用于划分三角面
def find_edge_center(edge):
    vertex1 = edge.verts[0]
    vertex2 = edge.verts[1]

    # 计算中点的坐标
    center = (vertex1.co + vertex2.co) / 2.0
    return center


# 定义一个函数，用于计算两个向量之间的角度
def angle_between_vectors(v1, v2):
    if v1.length > 0 and v2.length > 0:
        return v1.angle(v2)
    return 0.0


# 延伸特征线，从断点出发找到方向相近的下一个点
def extend_edges(breakpoint, selected_edges, optional_verts):
    adjacent_edges = [edge for edge in breakpoint.link_edges if edge.index in selected_edges]
    if len(adjacent_edges) == 1:
        selected_vertex = [v for v in adjacent_edges[0].verts if v != breakpoint][0]
        reference_vector = breakpoint.co - selected_vertex.co

        # 获取断点的法线方向
        breakpoint_normal = breakpoint.normal

        closest_vertex = None
        closest_angle = float('inf')

        # 找到方向最接近的顶点
        for vert in optional_verts:
            if vert == breakpoint and vert == selected_vertex:
                continue
            vector = vert.co - breakpoint.co

            # 计算向量在法线方向上的投影
            projected_vector = vector - vector.dot(breakpoint_normal) * breakpoint_normal
            #                angle = angle_between_vectors(projected_vector, reference_vector)

            angle = angle_between_vectors(vector, reference_vector)

            if angle < closest_angle:
                closest_angle = angle
                closest_vertex = vert

        if closest_vertex is not None:
            # 检查两个顶点之间是否已经存在边
            existing_edge = None
            for edge in bm.edges:
                if (edge.verts[0] == closest_vertex and edge.verts[1] == breakpoint) or (
                        edge.verts[0] == breakpoint and edge.verts[1] == closest_vertex):
                    existing_edge = edge
                    break

            if existing_edge is None:
                # 寻找 closest_vertex 的邻接点
                adjacent_verts_closest_vertex = [edge.other_vert(closest_vertex) for edge in closest_vertex.link_edges]

                # 寻找 breakpoint 的邻接点
                adjacent_verts_breakpoint = [edge.other_vert(breakpoint) for edge in breakpoint.link_edges]

                # 寻找 closest_vertex 和 breakpoint 的共同邻接点
                common_adjacent_verts = list(set(adjacent_verts_closest_vertex).intersection(adjacent_verts_breakpoint))

                if len(common_adjacent_verts) == 2:
                    # 寻找由共同邻接点组成的边
                    common_edge = None
                    for edge in common_adjacent_verts[0].link_edges:
                        if common_adjacent_verts[1] in edge.verts:
                            common_edge = edge
                            break

                    if common_edge:
                        center_pos = find_edge_center(common_edge)
                        if center_pos is not None:
                            print("create")
                            # 将交点添加到模型中
                            bm.verts.ensure_lookup_table()
                            # 位置
                            center_vert = bm.verts.new(center_pos)

                            # 创建新边，并设置为选中状态
                            selected_edge1 = bm.edges.new([center_vert, breakpoint])
                            selected_edge2 = bm.edges.new([center_vert, closest_vertex])

                            origin_edge1 = bm.edges.new([center_vert, common_edge.verts[0]])
                            origin_edge2 = bm.edges.new([center_vert, common_edge.verts[1]])

                            selected_edge1.select = True
                            selected_edges.append(selected_edge1.index)

                            selected_edge2.select = True
                            selected_edges.append(selected_edge2.index)

                            if common_edge.select:
                                origin_edge1.select = True
                                origin_edge2.select = True

                            # 创建面，连接新的中心顶点与断点和最近顶点
                            bm.faces.new([center_vert, breakpoint, common_edge.verts[0]])
                            bm.faces.new([center_vert, breakpoint, common_edge.verts[1]])

                            bm.faces.new([center_vert, closest_vertex, common_edge.verts[0]])
                            bm.faces.new([center_vert, closest_vertex, common_edge.verts[1]])

                            # 从 BMesh 对象中删除这条边
                            bm.edges.remove(common_edge)

                    else:
                        print("No common edge found.")
                else:
                    print("No common adjacent verts found.")
            else:
                existing_edge.select = True
                selected_edges.append(existing_edge.index)

            return closest_vertex
        return None
    return None


# 切换到对象模式
bpy.ops.object.mode_set(mode='OBJECT')

# 创建一个字典来存储边的权重
edge_weights = {edge.index: 0.0 for edge in bm.edges}

# 首先计算每个连通区域的断点并存储在字典中
component_breakpoints = {}

for component_info in connected_components_info:
    component_edges = component_info["edges"]

    # 创建该连通区域的断点集合
    breakpoints = find_breakpoints_for_component(bm, component_edges)

    component_breakpoints[component_info["index"]] = breakpoints

# 遍历每个连通区域，考虑断点并进行延伸
for component_info in connected_components_info:
    component_edges = component_info["edges"]
    component_index = component_info["index"]

    component_weight = component_info["weight"]
    for edge_index in component_edges:
        # 检查当前权重与新权重的大小，选择较大的值
        edge_weights[edge_index] = max(edge_weights.get(edge_index, 0.0), component_weight)

    # 获取该连通区域的断点
    breakpoints = component_breakpoints.get(component_index, set())

    # 遍历所有断点
    while breakpoints:
        #        print("breakpoints:", breakpoints)
        new_breakpoints = set()
        for breakpoint in breakpoints:
            if breakpoint in boundary_vertices:
                continue
            optional_verts = find_all_verts(breakpoint)
            #            print("optional_verts",len(optional_verts))
            #            print("component_edges(before):", len(component_edges))
            extend_points = extend_edges(breakpoint, component_edges, optional_verts)
            #            print("component_edges(after):", len(component_edges))
            if extend_points is not None:
                if extend_points in boundary_vertices:
                    continue
                else:
                    # 更新延伸出的边的权重，将其设置为连通区域的权重，保留较大的权重
                    for edge_index in component_edges:
                        if edge_index not in edge_weights:
                            edge_weights[edge_index] = component_weight
                        else:
                            edge_weights[edge_index] = max(edge_weights[edge_index], component_weight)
                    new_breakpoints.add(extend_points)

        # 更新连通区域信息，包括新的边
        component_info["edges"] = component_edges  # 这里可以不更新边，因为component_edges在extend_edges内部已经修改
        component_info["weight"] = component_weight
        breakpoints = new_breakpoints

# print("edge_weights:",edge_weights)
# 更新网格
bm.to_mesh(mesh)
mesh.update()
# 切换到编辑模式
bpy.ops.object.mode_set(mode='EDIT')

# 导出边的权重信息到 JSON 文件
output_weights_file_path = "export/edge_weights.json"
with open(output_weights_file_path, "w") as json_file:
    json.dump(edge_weights, json_file, indent=4)

print("Edge weights exported to", output_weights_file_path)

bm.free()
print("over")